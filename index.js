...
function filterStudentsByGrade(students, grade) {
  ...
  for (let elem of students) {
    if (elem.grade === grade) console.log(elem.name);
  }
}
...
function logArrayElements(arr) {
  ...
  arr.forEach((elem, index) => console.log(`Елемент ${index}:  ${elem}`));
}
...
function compareArrays(arr1, arr2) {
  ...
  console.assert(str1 === str2, "Масиви не однакові!");
}
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      elem[j] = Math.floor(Math.random() * 10);
    }
    arr[i] = elem;
  }
...
function countVowelsAndConsonants(word) {
  ...
  for (let char of word) {
    ...
    if (vowels.includes(char)) console.count("Vowels");
    else console.count("Consonant");
  }
